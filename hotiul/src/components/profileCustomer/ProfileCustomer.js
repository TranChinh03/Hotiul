...
const ProfileCustomer = props => {
	...
	function handleSave() {
		...
		UpdateCustomer(props.data.ID, state.ctzId, state.gender, state.name, state.phone);
		handleAction();
	}
	...
	function handleAdd() {
		...
		if (edit.ctzId === '' || edit.gender === '' || edit.phone === '' || edit.name === '') {
			...
			<div className={styles.container}>
				...
				<div className={styles.con2}>
					<table
						...
						<tbody className=" h-48">
							{bookings.slice(pageIndex * 9 - 9, pageIndex * 9).map((val, key) => {
								return (
									<tr
										...
										<td className={styles.col}>
											<button
												onClick={() => {
													setOnDetail(true);
													setSelectedBooking(props.data.Booking.find(x => x.ID === val.id));
				...
				<div className={styles.con1}>
					<p className=" text-mainColor pt-5">
						Showing <strong> 1 - {totalPage} </strong> results of <strong>{bookings.length}</strong>
					</p>
